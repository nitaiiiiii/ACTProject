...
import com.ci.act.R
import com.ci.act.widgets.sneaker.interfaces.OnSneakerClickListener
import com.ci.act.widgets.sneaker.interfaces.OnSneakerDismissListener
...
class Sneaker(private var context: Context) : View.OnClickListener, LifecycleObserver {
    ...
    fun sneakWithCustomColorWhiteText(color: String?) {
    ...
    fun sneakWithCustomColorBlackText(color: String?) {
        mBackgroundColor = Color.parseColor(color)
        mTitleColor = Color.parseColor("#000000")
        mMessageColor = Color.parseColor("#000000")
        mIconColorFilterColor = Color.parseColor("#000000")
...
//                Utils.convertToDp(context, mHeight.toFloat())
            ...
            with(sneakerView) {
                ...
                setPadding(46, context.dimen(R.dimen.dp40), 46, 50)
            ...
            if (mAutoHide) {
                val handler = Handler()
                handler.removeCallbacks(runnableView)
                handler.postDelayed(runnableView, mDuration.toLong())
            }
        }
    }

    private val runnableView = Runnable {
        removeView(sneakerView)
        mDismissListener?.onDismiss()
    }
    ...
    private val swipeGesture = object : OnSwipeTouchListener() {
